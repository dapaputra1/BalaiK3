...
function Resolve-Worksheet {
    ...
    foreach ($sheet in $sheets) {
        ...
        if ($name -match 'hcl|klorida|chloride') {
            return @{
                sheetName = $name
    ...
    throw 'Sheet acuan HCL Emisi tidak ditemukan.'
}
...
function Extract-FormulaConstants {
    ...
    if ($normalized -match '[A-Z]+\d+\*[A-Z]+\d+\*([0-9]+(?:\.[0-9]+)?)\*([0-9]+(?:\.[0-9]+)?)\*\(([0-9]+(?:\.[0-9]+)?)\+[A-Z]+\d+\)\*([0-9]+(?:\.[0-9]+)?)\/\([A-Z]+\d+\*[A-Z]+\d+\*([0-9]+(?:\.[0-9]+)?)\*([0-9]+(?:\.[0-9]+)?)\*[A-Z]+\d+\)') {
        $defaults.correctionFactor = [double] $Matches[1]
        $defaults.massFactor = [double] $Matches[2]
        $defaults.temperatureOffset = [double] $Matches[3]
        $defaults.pressureFactor = [double] $Matches[4]
        $defaults.divisorFactor = [double] $Matches[5]
        $defaults.denominatorTemperature = [double] $Matches[6]
    }
...
try {
    ...
    $formulaKadar = [string] (Get-CellValue -Cells $cells -Reference 'I42' -Field 'formula')
    if (-not $formulaKadar) {
        $formulaKadar = [string] (Get-CellValue -Cells $cells -Reference 'I33' -Field 'formula')
    }
    ...
    $result = @{
        ...
        lod = @{
            ...
            tm = To-Number (Get-CellValue -Cells $cells -Reference 'G42')
            p = To-Number (Get-CellValue -Cells $cells -Reference 'H42')
            kadar = To-Number (Get-CellValue -Cells $cells -Reference 'I42')
        }
        sample = @{
            ...
            tm = To-Number (Get-CellValue -Cells $cells -Reference 'G33')
            p = To-Number (Get-CellValue -Cells $cells -Reference 'H33')
            kadar = To-Number (Get-CellValue -Cells $cells -Reference 'I33')
        }
        constants = $constants
    }
