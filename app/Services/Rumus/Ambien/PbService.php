...
class PbService extends BasePrepanalisaRumusService
{
    public function __construct(
        private readonly PbExcelReferenceService $referenceService
    ) {
    }
    ...
    private function normalizeHasilBaca(array $dataset): array
    {
        ...
        foreach ($rows as $index => $row) {
            ...
            if (in_array($label, ['blk', 'blanko'], true) && $kand !== null) {
                $blankoValues[] = $kand;
            }
        ...
        foreach ($rows as $index => $row) {
            ...
            if ($label === 'rata-rata') {
                if ($avgBlanko !== null) {
                    $row['kand-spl'] = $this->formatDecimal($avgBlanko, 4);
                    $row['knd-bl'] = '';
                    if (isset($row['cols']) && is_array($row['cols'])) {
                        $row['cols'][4] = $row['kand-spl'];
                        $row['cols'][5] = '';
            ...
            $kand = $this->toNullableFloat($row['kand-spl'] ?? null);
            $row['knd-bl'] = ($kand !== null && $avgBlanko !== null)
                ? $this->formatDecimal($kand - $avgBlanko, 4)
                : '';
            if (isset($row['cols']) && is_array($row['cols'])) {
                $row['cols'][5] = $row['knd-bl'];
            }
            $rows[$index] = $row;
        }
