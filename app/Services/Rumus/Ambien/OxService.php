...
class OxService extends BasePrepanalisaRumusService
{
    public function __construct(
        private readonly OxExcelReferenceService $referenceService,
    ...
    private function normalizeHasilBaca(array $dataset): array
    {
        ...
        foreach ($rows as $index => $row) {
            ...
            $kand = $this->absorbanceReferenceService->calculate('OX', $abs);
        ...
        foreach ($rows as $index => $row) {
            ...
            $kand = $this->toNullableFloat($row['kand'] ?? null);
            if ($kand === null || $avgBlanko === null) {
                $row['kndbl'] = '';
            } else {
                $row['kndbl'] = $this->formatDecimal($kand - $avgBlanko, 4);
            }
    ...
    private function normalizeCalcRow(array $row, array $lodReference): array
    {
        ...
        $row['kadar_ppm'] = $this->formatDecimal($ppm, 6);
