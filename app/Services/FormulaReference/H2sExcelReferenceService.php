...
class H2sExcelReferenceService
{
    private const STORAGE_PATH = 'reference_h2s.xlsx';
    private const FALLBACK_EXTERNAL_PATH = 'C:\Users\user\OneDrive\Documents\H2S.xlsx';
    ...
    public function getReference(): array
    {
        ...
        return Cache::rememberForever('prepanalisa:formula-reference:h2s:' . $signature, function () use ($workbookPath) {
            try {
        ...
        $lodRows = ParameterLod::query()
            ...
            ->whereHas('serviceParameter', function ($query) {
                $query->whereIn('system_code', [
                    ServiceSystemCode::parameter(ServiceSystemCode::CATEGORY_LK, 'H2S'),
                    ServiceSystemCode::parameter(ServiceSystemCode::CATEGORY_AMB, 'H2S1'),
                ]);
            })
            ->get();
    ...
    private function buildLodRowFromModel(
        ...
        $kons = $this->normalizeFloat($lod->kons, (float) ($fallback['kons'] ?? 0.0002));
        $vol = $this->normalizeFloat($lod->vol, (float) ($fallback['vol'] ?? 10.0));
        $fr = $this->normalizeFloat($lod->fr, (float) ($fallback['fr'] ?? 1.5));
    ...
    {
        $candidates = [
            env('PREPANALISA_H2S_REFERENCE_PATH'),
    ...
    private function extractReference(string $workbookPath): array
    {
        if (!class_exists(ZipArchive::class)) {
            throw new RuntimeException('ZipArchive tidak tersedia untuk membaca acuan Excel H2S.');
        }

        $zip = new ZipArchive();
        if ($zip->open($workbookPath) !== true) {
            throw new RuntimeException('File acuan Excel H2S tidak dapat dibuka.');
        }
        ...
        try {
            ...
            return [
                'sheetName' => $sheetName,
                'factorVolume' => $this->extractFactorVolume($cells['I32']['formula'] ?? ($cells['I29']['formula'] ?? '')),
                'molecularWeight' => $this->extractMolecularWeight($cells['I32']['formula'] ?? ($cells['I29']['formula'] ?? '')),
                ...
                'lodAmbien' => $this->extractRow($cells, 32),
                'lodLk' => $this->extractRow($cells, 39),
            ];
        } finally {
            $zip->close();
        }
    ...
    private function resolveWorksheet(ZipArchive $zip): array
    {
        ...
        if ($workbookXml === false || $relsXml === false) {
            throw new RuntimeException('Metadata workbook acuan Excel H2S tidak lengkap.');
        }

        $workbook = simplexml_load_string($workbookXml);
        $rels = simplexml_load_string($relsXml);
        if ($workbook === false || $rels === false) {
            throw new RuntimeException('Workbook acuan Excel H2S tidak valid.');
        }
        ...
        foreach ($workbook->xpath('//a:sheets/a:sheet') ?: [] as $sheet) {
            ...
            if (stripos($name, 'h2s') !== false) {
                return [$name, $targets[$relationId]];
            }
        }

        throw new RuntimeException('Sheet acuan H2S tidak ditemukan.');
    }
    ...
    private function readWorksheetCells(ZipArchive $zip, string $sheetPath, array $sharedStrings): array
    {
        ...
        if ($xml === false) {
            throw new RuntimeException('Worksheet acuan H2S tidak dapat dibaca.');
        }

        $sheet = simplexml_load_string($xml);
        if ($sheet === false) {
            throw new RuntimeException('Worksheet acuan H2S tidak valid.');
        }
    ...
    {
        if (preg_match('/\*760\*([0-9]+(?:\.[0-9]+)?)\//', preg_replace('/\s+/', '', strtoupper($formula)), $matches)) {
    ...
    {
        if (preg_match('/\*[0-9]+(?:\.[0-9]+)?\*([0-9]+(?:\.[0-9]+)?)\)$/', preg_replace('/\s+/', '', strtoupper($formula)), $matches)) {
            return (float) $matches[1];
        }
