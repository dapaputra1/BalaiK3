...
class BenzeneExcelReferenceService
{
    ...
    public function getReference(): array
    {
        ...
        $reference = $workbookPath === null
            ...
            : Cache::rememberForever(
                'prepanalisa:formula-reference:benzene:' . md5($workbookPath . '|' . (string) @filesize($workbookPath) . '|' . (string) @filemtime($workbookPath)),
    ...
    private function applyDatabaseLodOverride(array $reference): array
    {
        $row = ParameterLod::query()
            ...
            ->whereHas('serviceParameter', function ($query) {
                $query->whereIn('system_code', [
                    ServiceSystemCode::parameter(ServiceSystemCode::CATEGORY_LK, 'BENZ'),
                    ServiceSystemCode::parameter(ServiceSystemCode::CATEGORY_LK, 'HC'),
                    ServiceSystemCode::parameter(ServiceSystemCode::CATEGORY_AMB, 'BENZ1'),
                    ServiceSystemCode::parameter(ServiceSystemCode::CATEGORY_AMB, 'HC1'),
                ]);
            })
            ->latest('updated_at')
            ->first();
        ...
        $kons = $this->normalizeFloat($row->kons, (float) ($reference['lod']['kons'] ?? 0.00394));
...
try {
  ...
  $result = @{
    ...
    yValue = [string]$ws.Cells.Item(24,5).Value2
    xValue = [string]$ws.Cells.Item(25,5).Value2
    formulaY = 'Y = ' + [string]$ws.Cells.Item(24,5).Value2
    formulaX = [string]$ws.Cells.Item(25,5).Formula
    formulaConc = [string]$ws.Cells.Item(21,6).Formula
    formulaPpm = [string]$ws.Cells.Item(40,11).Formula
    formulaUgm3 = [string]$ws.Cells.Item(40,14).Formula
    lod = @{
      kons = [string]$ws.Cells.Item(40,3).Value2
      ...
      ppm = [string]$ws.Cells.Item(40,11).Value2
      ugm3 = [string]$ws.Cells.Item(40,14).Value2
    }
  }
...
POWERSHELL;
        ...
        $scriptPath = tempnam(sys_get_temp_dir(), 'benzene-ref-');
        if ($scriptPath === false) {
            throw new RuntimeException('Gagal menyiapkan parser Benzene.');
        }
        ...
        if (!is_array($decoded)) {
            throw new RuntimeException('Output parser Benzene tidak valid.');
        }

        $ppmFormula = trim((string) ($decoded['formulaPpm'] ?? ''));
        $ugm3Formula = trim((string) ($decoded['formulaUgm3'] ?? ''));
    ...
    {
        if (preg_match('/\*[0-9]+(?:\.[0-9]+)?\*([0-9]+(?:\.[0-9]+)?)\*760\/\(/', preg_replace('/\s+/', '', strtoupper($formula)), $m)) {
            return (float) $m[1];
        }
    ...
    private function normalizeReference(array $payload, string $workbookPath): array
    {
        ...
        $reference['lod'] = [
            ...
            'ugm3' => $this->formatDecimal(data_get($payload, 'lod.ugm3'), self::DEFAULT_REFERENCE['lod']['ugm3'], 4),
        ];
