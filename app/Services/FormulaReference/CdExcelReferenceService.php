...
class CdExcelReferenceService
{
    private const SERVICE_PARAMETER_SYSTEM_CODE = 'LK__KDLCD';
    private const STORAGE_PATH = 'reference_cd.xlsx';
    private const FALLBACK_EXTERNAL_PATHS = [
        'C:\Users\user\OneDrive\Cd.xlsx',
        'C:\Users\user\OneDrive\Documents\Cd.xlsx',
        'C:\Users\user\OneDrive\Documents\Cadmium (Cd).xlsx',
    ];
    ...
    private const DEFAULT_REFERENCE = [
        ...
        'standardSeries' => [0.25, 0.5, 0.75, 1.0, 1.5],
        'lod' => [
            'kons' => '',
            'vol' => '',
            'fr' => '',
    ...
    public function getReference(): array
    {
        ...
        $reference = $workbookPath === null
            ...
            : Cache::rememberForever(
                'prepanalisa:formula-reference:cd:' . md5($workbookPath . '|' . (string) @filesize($workbookPath) . '|' . (string) @filemtime($workbookPath)),
    ...
    {
        $absorbance = $this->absorbanceReferenceService->getReference('CD');
    ...
    private function extractReference(string $workbookPath): array
    {
        if (!class_exists(ZipArchive::class)) {
            throw new RuntimeException('ZipArchive tidak tersedia untuk membaca acuan Excel Cd.');
        }

        $zip = new ZipArchive();
        if ($zip->open($workbookPath) !== true) {
            throw new RuntimeException('File acuan Excel Cd tidak dapat dibuka.');
        }
        ...
        try {
            ...
            $curveY = $this->toNullableFloat($cells['G24']['value'] ?? null);
            $curveX = $this->toNullableFloat($cells['G25']['value'] ?? null);
    ...
    {
        $candidates = [
            env('PREPANALISA_CD_REFERENCE_PATH'),
    ...
    private function resolveWorksheet(ZipArchive $zip): array
    {
        ...
        if ($workbookXml === false || $relsXml === false) {
            throw new RuntimeException('Metadata workbook acuan Excel Cd tidak lengkap.');
        }

        $workbook = simplexml_load_string($workbookXml);
        $rels = simplexml_load_string($relsXml);
        if ($workbook === false || $rels === false) {
            throw new RuntimeException('Workbook acuan Excel Cd tidak valid.');
        }
        ...
        foreach ($workbook->xpath('//a:sheets/a:sheet') ?: [] as $sheet) {
            ...
            if (stripos($name, 'cd') !== false || stripos($name, 'cadmium') !== false) {
        ...
        throw new RuntimeException('Sheet acuan Cd tidak ditemukan.');
    }
    ...
    private function readWorksheetCells(ZipArchive $zip, string $sheetPath, array $sharedStrings): array
    {
        ...
        if ($xml === false) {
            throw new RuntimeException('Worksheet acuan Excel Cd tidak ditemukan.');
        }

        $sheet = simplexml_load_string($xml);
        if ($sheet === false) {
            throw new RuntimeException('Worksheet acuan Excel Cd tidak valid.');
        }
