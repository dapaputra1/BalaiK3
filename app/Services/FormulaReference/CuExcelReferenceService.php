...
class CuExcelReferenceService
{
    private const SERVICE_PARAMETER_SYSTEM_CODE = 'LK__KDLCU';
    private const STORAGE_PATH = 'reference_cu.xlsx';
    private const FALLBACK_EXTERNAL_PATHS = [
        'C:\Users\user\OneDrive\Documents\Cu.xlsx',
        'C:\Users\user\OneDrive\Documents\Copper (Cu).xlsx',
    ];
    ...
    private const DEFAULT_REFERENCE = [
        ...
        'standardSeries' => [0.1, 0.5, 1.5, 3.0, 5.0],
        'lod' => [
            'kons' => '',
            'vol' => '',
            'fr' => '',
    ...
    public function getReference(): array
    {
        ...
        $reference = $workbookPath === null
            ...
            : Cache::rememberForever(
                'prepanalisa:formula-reference:cu:' . md5($workbookPath . '|' . (string) @filesize($workbookPath) . '|' . (string) @filemtime($workbookPath)),
    ...
    {
        $absorbance = $this->absorbanceReferenceService->getReference('CU');
    ...
    private function extractReference(string $workbookPath): array
    {
        if (!class_exists(ZipArchive::class)) {
            throw new RuntimeException('ZipArchive tidak tersedia untuk membaca acuan Excel Cu.');
        }

        $zip = new ZipArchive();
        if ($zip->open($workbookPath) !== true) {
            throw new RuntimeException('File acuan Excel Cu tidak dapat dibuka.');
        }
        ...
        try {
            ...
            $curveY = $this->toNullableFloat($cells['G27']['value'] ?? null);
            $curveX = $this->toNullableFloat($cells['G28']['value'] ?? null);
    ...
    {
        $values = [];
        foreach (['F17', 'F19', 'F20', 'F21', 'F22'] as $ref) {
            $value = $this->toNullableFloat($cells[$ref]['value'] ?? null);
            if ($value !== null) {
                $values[] = $value;
            }
        }
    ...
    {
        $candidates = [
            env('PREPANALISA_CU_REFERENCE_PATH'),
    ...
    private function resolveWorksheet(ZipArchive $zip): array
    {
        ...
        if ($workbookXml === false || $relsXml === false) {
            throw new RuntimeException('Metadata workbook acuan Excel Cu tidak lengkap.');
        }

        $workbook = simplexml_load_string($workbookXml);
        $rels = simplexml_load_string($relsXml);
        if ($workbook === false || $rels === false) {
            throw new RuntimeException('Workbook acuan Excel Cu tidak valid.');
        }
        ...
        foreach ($workbook->xpath('//a:sheets/a:sheet') ?: [] as $sheet) {
            ...
            if (stripos($name, 'cu') !== false || stripos($name, 'copper') !== false) {
        ...
        throw new RuntimeException('Sheet acuan Cu tidak ditemukan.');
    }
    ...
    private function readWorksheetCells(ZipArchive $zip, string $sheetPath, array $sharedStrings): array
    {
        ...
        if ($xml === false) {
            throw new RuntimeException('Worksheet acuan Excel Cu tidak ditemukan.');
        }

        $sheet = simplexml_load_string($xml);
        if ($sheet === false) {
            throw new RuntimeException('Worksheet acuan Excel Cu tidak valid.');
        }
