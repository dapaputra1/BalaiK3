...
class AsExcelReferenceService
{
    private const SERVICE_PARAMETER_SYSTEM_CODE = 'LK__KDLAS';
    private const STORAGE_PATH = 'reference_as.xlsx';
    private const FALLBACK_EXTERNAL_PATHS = [
        'C:\Users\user\OneDrive\Documents\As.xlsx',
        'C:\Users\user\OneDrive\Documents\Arsenic (As).xlsx',
    ];
    ...
    private const DEFAULT_REFERENCE = [
        ...
        'standardSeries' => [1.0, 3.0, 5.0, 7.0, 10.0],
        'lod' => [
            'kons' => '',
            'vol' => '',
            'fr' => '',
    ...
    public function getReference(): array
    {
        ...
        $reference = $workbookPath === null
            ...
            : Cache::rememberForever(
                'prepanalisa:formula-reference:as:' . md5($workbookPath . '|' . (string) @filesize($workbookPath) . '|' . (string) @filemtime($workbookPath)),
    ...
    {
        $absorbance = $this->absorbanceReferenceService->getReference('AS');
    ...
    private function extractReference(string $workbookPath): array
    {
        if (!class_exists(ZipArchive::class)) {
            throw new RuntimeException('ZipArchive tidak tersedia untuk membaca acuan Excel As.');
        }

        $zip = new ZipArchive();
        if ($zip->open($workbookPath) !== true) {
            throw new RuntimeException('File acuan Excel As tidak dapat dibuka.');
        }
    ...
    {
        $candidates = [
            env('PREPANALISA_AS_REFERENCE_PATH'),
    ...
    private function resolveWorksheet(ZipArchive $zip): array
    {
        ...
        if ($workbookXml === false || $relsXml === false) {
            throw new RuntimeException('Workbook Excel As tidak lengkap.');
        }

        $workbook = simplexml_load_string($workbookXml);
        $rels = simplexml_load_string($relsXml);
        if ($workbook === false || $rels === false) {
            throw new RuntimeException('Workbook Excel As tidak dapat diparse.');
        }
        ...
        throw new RuntimeException('Worksheet Excel As tidak ditemukan.');
    }
    ...
    private function readWorksheetCells(ZipArchive $zip, string $sheetPath, array $sharedStrings): array
    {
        ...
        if ($xml === false) {
            throw new RuntimeException('Worksheet Excel As tidak dapat dibaca.');
        }

        $worksheet = simplexml_load_string($xml);
        if ($worksheet === false) {
            throw new RuntimeException('Worksheet Excel As tidak dapat diparse.');
        }
