...
class HgAasExcelReferenceService
{
    private const SERVICE_PARAMETER_SYSTEM_CODE = 'LK__KDLHG';
    private const STORAGE_PATH = 'reference_hg_aas.xlsx';
    private const FALLBACK_EXTERNAL_PATHS = [
        'C:\Users\user\OneDrive\Documents\Hg 2.xlsx',
        'C:\Users\user\OneDrive\Hg 2.xlsx',
    ];
    ...
    private const DEFAULT_REFERENCE = [
        ...
        'standardSeries' => [10.0, 20.0, 30.0, 40.0, 50.0],
        'lod' => [
            'kons' => '',
            'vol' => '',
            'fr' => '',
    ...
    public function getReference(): array
    {
        ...
        $reference = $workbookPath === null
            ...
            : Cache::rememberForever(
                'prepanalisa:formula-reference:hg-aas:' . md5($workbookPath . '|' . (string) @filesize($workbookPath) . '|' . (string) @filemtime($workbookPath)),
    ...
    {
        $absorbance = $this->absorbanceReferenceService->getReference('HG');
    ...
    private function extractReference(string $workbookPath): array
    {
        if (!class_exists(ZipArchive::class)) {
            throw new RuntimeException('ZipArchive tidak tersedia untuk membaca acuan Excel Hg AAS.');
        }

        $zip = new ZipArchive();
        if ($zip->open($workbookPath) !== true) {
            throw new RuntimeException('File acuan Excel Hg AAS tidak dapat dibuka.');
        }
    ...
    {
        $candidates = [
            env('PREPANALISA_HG_AAS_REFERENCE_PATH'),
    ...
    private function resolveWorksheet(ZipArchive $zip): array
    {
        ...
        if ($workbookXml === false || $relsXml === false) {
            throw new RuntimeException('Metadata workbook acuan Excel Hg AAS tidak lengkap.');
        }

        $workbook = simplexml_load_string($workbookXml);
        $rels = simplexml_load_string($relsXml);
        if ($workbook === false || $rels === false) {
            throw new RuntimeException('Workbook acuan Excel Hg AAS tidak valid.');
        }
        ...
        foreach ($workbook->xpath('//a:sheets/a:sheet') ?: [] as $sheet) {
            ...
            if (stripos($name, 'hg') !== false || stripos($name, 'merkuri') !== false || stripos($name, 'mercury') !== false) {
        ...
        throw new RuntimeException('Sheet acuan Hg AAS tidak ditemukan.');
    }
    ...
    private function readWorksheetCells(ZipArchive $zip, string $sheetPath, array $sharedStrings): array
    {
        ...
        if ($xml === false) {
            throw new RuntimeException('Worksheet acuan Excel Hg AAS tidak ditemukan.');
        }

        $sheet = simplexml_load_string($xml);
        if ($sheet === false) {
            throw new RuntimeException('Worksheet acuan Excel Hg AAS tidak valid.');
        }
