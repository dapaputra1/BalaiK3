        ...
        >
          ...
          <div class="border rounded-3 p-3 bg-light mt-3">
            <div class="d-flex align-items-start justify-content-between flex-wrap gap-2">
              <div>
                <div class="small text-muted">Dokumen Kuitansi (PDF)</div>
                <div class="fw-semibold">Kuitansi pembayaran parameter setelah pengujian</div>
                @if(($order['kuitansi_status'] ?? 'belum_kirim') === 'terkirim')
                  <div class="small text-muted mt-1">Dikirim: {{ $order['kuitansi_sent_at'] ?? '-' }}</div>
                @endif
                <a href="{{ $order['invoice_url'] ?? '#' }}" target="_blank" rel="noopener" class="btn btn-outline-success btn-sm mt-2">
                  Lihat Kuitansi
                </a>
              </div>
              ...
              <div class="d-flex gap-2 flex-wrap">
                ...
                @else
                  <button
                    ...
                    @if(empty($order['invoice_url'])) disabled @endif
                  >
                    Teruskan Kuitansi & Lanjut ke Suket
                  </button>
                @endif
              </div>
            </div>
    ...
    if (submitBtn) {
      ...
      const proceed = window.Swal
        ? await window.Swal.fire({
            ...
            text: 'Setelah kuitansi diteruskan, permohonan akan lanjut ke tahap penerbitan suket.',
            showCancelButton: true,
            confirmButtonText: 'Ya, teruskan',
            cancelButtonText: 'Batal',
          }).then((r) => r.isConfirmed)
        : confirm('Teruskan kuitansi ke pelanggan sekarang?');
